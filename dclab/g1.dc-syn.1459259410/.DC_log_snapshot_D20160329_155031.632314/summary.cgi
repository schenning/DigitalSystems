...
#END DEBUG VARIABLES
    ...
    def processLine(line):
        ...
        if match:
            ...
            if not messages[type].has_key(tag):
                messages[type][tag] = {}
                messages[type][tag][match.group(0)] = 1
                tagCounts[type+tag] = 1
            else:
                if not messages[type][tag].has_key(match.group(0)):
                    messages[type][tag][match.group(0)] = 1
                else:
                    messages[type][tag][match.group(0)] += 1
                tagCounts[type+tag] += 1
    ...
    def run():
        ...
        try:
            ...
            orderedMessages = {}
            for type in types:
                orderedTags = messages[type].keys()
                orderedTags.sort()
            ...
            for type in types:
                ...
                for tag in orderedMessages[type]:
                    ...
                    for line,count in messages[type][tag].iteritems():
